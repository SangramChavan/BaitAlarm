...
public class CSSParser {
	public static void main(String[] args) {
		...
		selectTagTypes("D:\\work\\project\\TaxiRecommender\\TFinderWeb\\WebContent\\theme\\css\\style.css");
	}
	public static CSSModel selectTagTypes(String cssFilePath)
	{
		...
		String tagsCSV= tagSelector.toString().substring(1, tagSelector.toString().length() - 1).replace(", ", ",");
		String classesCSV= classSelector.toString().substring(1, classSelector.toString().length() - 1).replace(", ", ",");
		String idsCSV= idSelector.toString().substring(1, idSelector.toString().length() - 1).replace(", ", ",");
		String othersCSV= otherSelector.toString().substring(1, otherSelector.toString().length() - 1).replace(", ", ",");
...
System.out.println("LOading CSS "+filePath);
			if (!filePath.startsWith("http")) {
				...
				if(filePath.toLowerCase().startsWith("http:"))
					...
					stream = HttpsURLReader.readHttpsURLStream(filePath);
			}
			// wrap as an InputSource
